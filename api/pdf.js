...
// Map para armazenamento serverless dos PDFs gerados por código
const pdfStore = new Map();
const studentDataStore = new Map();
...
module.exports = async function handler(req, res) {
  ...
  // POST: Salvar PDF pré-gerado ou Dados do Estudante
  if (req.method === 'POST') {
    try {
      const body = typeof req.body === 'string' ? JSON.parse(req.body) : req.body;
      const studentCode = (body.code || code).toLowerCase();

      if (body.pdfBase64) {
        const base64Data = body.pdfBase64.replace(/^data:application\/pdf;base64,/, '');
        const pdfBuffer = Buffer.from(base64Data, 'base64');
        pdfStore.set(studentCode, pdfBuffer);
        return res.status(200).json({ success: true, code: studentCode });
      }

      if (body.nome || body.foto) {
        studentDataStore.set(studentCode, body);
      }
  ...
  // GET: Servir o PDF oficial armazenado ou gerado para o código
  if (req.method === 'GET') {
    try {
      // 1. Se já existe um PDF salvo em buffer para este código, serve ele diretamente
      ...
      const storedData = studentDataStore.get(code) || {};

      const studentNome = req.query.nome || req.query.n || storedData.nome || 'Emerson Vicosa de Lima';
      const studentCurso = req.query.curso || req.query.c || storedData.curso || 'Ciência da Computação';
      const studentInst = req.query.instituicao || req.query.i || storedData.instituicao || 'UNIRITTER';
      const rawCpf = req.query.cpf || storedData.cpf || '039.894.040-16';
      const cpf = rawCpf.length === 11 ? rawCpf.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4') : rawCpf;
      const nascimento = req.query.nascimento || req.query.d || storedData.nascimento || '10/08/1998';
      const fotoData = req.query.foto || req.query.f || storedData.foto;

      // Gerar QR Code ultraleve (somente o link limpo do PDF)
      const host = req.headers.host || 'carteira-estudante-pwa.vercel.app';
      const protocol = req.headers['x-forwarded-proto'] || 'https';
      const qrTargetUrl = `${protocol}://${host}/pdf/${code}.pdf`;
      const qrDataUrl = await QRCode.toDataURL(qrTargetUrl, { margin: 1, width: 300, errorCorrectionLevel: 'L' });
      ...
      if (fotoData && typeof fotoData === 'string' && fotoData.startsWith('data:image/')) {
        try {
          const isPng = fotoData.toLowerCase().includes('data:image/png');
          const format = isPng ? 'PNG' : 'JPEG';
          pdf.addImage(fotoData, format, 22, 72, 36, 46);
        } catch (e) {
...
module.exports.setPdfForCode = function(code, buffer) {
  if (!code || !buffer) return;
  pdfStore.set(code.toLowerCase(), buffer);
};
